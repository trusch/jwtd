...
func (s *JWTDSuite) TestGetHttpEchoOneAdminTokenWrongPassword() {
	token, err := s.GetToken("default", "admin", "wrong-password", "http-echo-1", "role", "admin")
	s.Error(err)
...
func (s *JWTDSuite) TestGetHttpEchoOneAdminTokenWrongLabel() {
	token, err := s.GetToken("default", "admin", "admin", "http-echo-1", "role", "wrong-label")
	s.Error(err)
	s.Equal("http error: 401", err.Error(), "wrong error")
	s.Empty(token, "admin token is not empty")
}
...
func (s *JWTDSuite) TestGetHttpEchoOneAdminTokenWrongService() {
	token, err := s.GetToken("default", "admin", "admin", "wrong-service", "role", "admin")
	s.Error(err)
	s.Equal("http error: 401", err.Error(), "wrong error")
	s.Empty(token, "admin token is not empty")
}
...
func (s *JWTDSuite) TestGetHttpEchoOneAdminTokenAsUser() {
	token, err := s.GetToken("default", "user", "user", "http-echo-1", "role", "admin")
	s.Error(err)
	s.Equal("http error: 401", err.Error(), "wrong error")
	s.Empty(token, "admin token is not empty")
}
